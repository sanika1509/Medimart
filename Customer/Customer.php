...
<body>
    <div class="container-fluid">
        <div class="row">
            ...
            <div class="col-12 col-sm-6 col-md-6 text-end" style="margin-bottom: 30px;"><a href= "../Options/Options.php" class="btn btn-primary" role="button" style=" margin-right:20px;"><i class="fa fa-arrow-left" ></i>&nbsp;Go To Ponentes</a><a href= "Add/CustomerForm.php" class="btn btn-primary" role="button" style=" margin-right:20px;"><i class="fa fa-plus" ></i>&nbsp;Add Customer</a></div>
        ...
        <div class="card" id="TableSorterCard">
            <div class="card-header py-3">
                ...
                {
                    $sql_all_customers = "select * from customers order by firstname";
                    $result = mysqli_query($conn,$sql_all_customers);
                    $total_rows_feched=mysqli_num_rows($result);
                }
                else if(isset($_POST["age_asc_filter"]))
                {
                    $sql_all_customers = "select * from customers order by age";
                    $result = mysqli_query($conn,$sql_all_customers);
                    $total_rows_feched=mysqli_num_rows($result);
                }
                else if(isset($_POST["age_desc_filter"]))
                {
                    $sql_all_customers = "select * from customers order by age desc";
                    $result = mysqli_query($conn,$sql_all_customers);
                    $total_rows_feched=mysqli_num_rows($result);
                }
                else if(isset($_POST["female_filter"]))
                {
                    $sql_all_customers = "select * from customers where gender='female'";
                    $result = mysqli_query($conn,$sql_all_customers);
                    $total_rows_feched=mysqli_num_rows($result);
                }
                else if(isset($_POST["male_filter"]))
                {
                    $sql_all_customers = "select * from customers where gender='male'";
                    $result = mysqli_query($conn,$sql_all_customers);
                    $total_rows_feched=mysqli_num_rows($result);
                }
                ?>
                        ...
                        <table class="table table-striped table tablesorter" id="ipi-table">
                            ...
                            <?php
                                ...
                                <tr>
                                    ...
                                    <a class="btn btnMaterial btn-flat success semicircle" role="button" href="Add/UpdateCustomer.php?id=<?php echo $row['customer_id'];?>"><i class="fas fa-pen"></i></a>
                                    <a class="btn btnMaterial btn-flat accent btnNoBorders checkboxHover" role="button" style="margin-left: 5px;"  data-bs-target="#delete-modal"  href="Add/DeleteCustomer.php?id=<?php echo $row['customer_id'];?>">
